...
interface SeaTripEditDrawerProps {
  open: boolean;
  onClose: () => void;
  trip: SeaTrip | null;
  onSuccess: () => void;
...
export default function SeaTripEditDrawer({ open, onClose, trip, onSuccess, authToken }: SeaTripEditDrawerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formdata.append("video", form.video_url);
      ...
      if (trip?.id) {
        ...
        url = `https://darkgray-termite-166434.hostingersite.com/api/seatrips/${trip.id}`;
        method = "POST";
      } else {
        // Create new trip
        url = `https://darkgray-termite-166434.hostingersite.com/api/seatrips`;
        method = "POST";
      }
  ...
  const handleDelete = async () => {
    ...
    if (!confirm('Are you sure you want to delete this sea trip?')) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`https://darkgray-termite-166434.hostingersite.com/api/seatrips/${trip.id}`, {
        method: "DELETE",
        headers: myHeaders,
        redirect: "follow"
      });
      ...
      setError(error.message || "Failed to delete sea trip");
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-30 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg relative">
        ...
        <h2 className="text-xl font-bold mb-4">{isEditing ? 'Edit' : 'Add New'} SeaTrip</h2>
        <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            {form.image_url && (
              <div className="mt-2">
                <img src={form.image_url} alt="Current" className="h-20 w-32 object-cover rounded" />
              </div>
            )}
          </div>
